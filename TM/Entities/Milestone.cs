...
public class Milestone : ProjectItem, ICloneable
{
    public Difficulty Difficulty { get; set; }
    public List<Task> Tasks { get; set; }

    public bool IsLeaf { get { return Tasks.Count == 0; } }

    public Milestone(string name, DateTime? duedate = null)
    {
        UUID = Guid.NewGuid();
    ...
    public Milestone(XmlNode milestone)
    {
        SetCommonElements(milestone);

        Tasks = new List<Task>();

        Difficulty = XMLhelper.GetInnerTextFromNode(milestone.ChildNodes, "Difficulty", false).ToEnumSafe<Difficulty>();

        XmlNode tasks = milestone.ChildNodes.FindNodeByName("tasks", false);
    ...
    public Milestone(NodeModel node)
    {
        ...
        {
            if (n.Parent == node.Id && n.NodeType == ProjectItemType.Task)
                Tasks.Add(new Task(n));
    ...
    public XmlDocument ToXml()
    {
        ...
        XmlElement milestone = doc.CreateElement("milestone");

        XmlAttribute uuid = doc.CreateAttribute("uuid");
        uuid.InnerText = UUID.ToString();
        milestone.Attributes.Append(uuid);
        ...
        if (Finished.HasValue)
        {
            ...
            milestone.AppendChild(finished);
        }
    ...
    {
        Milestone that = new Milestone(this.Name);
