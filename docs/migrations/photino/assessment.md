...
- `jnUtil` 2.0.0 supports `net10.0`; `TM.Core` now targets portable .NET 10. Preserve the compatible file formats and validate runtime behavior on native Linux/macOS hosts before claiming support.
- `ProjectCryptoService` uses Windows DPAPI for in-memory master and ECDH private keys on Windows and authenticated process-local protection elsewhere. Its persisted document/key/certificate encryption is a separate password-derived mechanism. Portable password file encryption replaces account-file encryption.
